...
describe('/oauth2/clients route tests', () => {
  ...
  it('should create a client with the provided credientials and resources', async () => {
    server = await resetAndMockDB(async (allDbs) => {
      ...
      allDbs.models.oauthClients.create = () => new Promise((resolve, reject) => resolve({
        get: () => oauthClient,
      }));
    });
  ...
  it('should validate the parameters correctly', async () => {
    server = await resetAndMockDB(async (allDbs) => {
      ...
      allDbs.models.oauthClients.create = () => new Promise((resolve, reject) => reject(new Error(errorMessage)));
    });
    const res = await server.inject({
      method: 'POST',
      url: '/oauth2/clients',
