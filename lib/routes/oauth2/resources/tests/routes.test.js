...
describe('/oauth2/resources route tests', () => {
  ...
  it('should return badRequest if request creation fails', async () => {
    server = await resetAndMockDB(async (allDbs) => {
      ...
      allDbs.models.oauthClientResources.create = () => new Promise((resolve, reject) => reject(new Error(errorMessage)));
    });
    const postPayload = {
      oauth_client_id: 1,
      resource_id: 1,
  ...
  it('should return badRequest if the parameters are wrong', async () => {
    server = await resetAndMockDB(async (allDbs) => {
      ...
      allDbs.models.oauthClientResources.update = () => new Promise((resolve, reject) => reject(new Error(errorMessage)));
    });
    const postPayload = {
      oauth_client_id: oauthClient.id,
      resource_id: 1,
