...
const errorMessage = `Unable top update scope: ${
    updatePayload.scope
} for oauthClient: ${updatePayload.oauth_client_id}`;
...
describe('/oauth2/scopes route tests', () => {
    let server;
    it.only('should return the scope correctly', async () => {
        server = await resetAndMockDB(async allDbs => {
            allDbs.oauth_client_scopes.$queryInterface.$useHandler(function(
                query
            ) {
                if (query === 'findById') {
                    return oauthClient;
    ...
    it('should update the scope of client', async () => {
        server = await resetAndMockDB(async allDbs => {
            ...
            allDbs.oauth_client_scopes.update = () =>
                new Promise((resolve, reject) => resolve(oauthClient));
        });
        const res = await server.inject({
            method: 'PATCH',
            url: '/oauth2/scopes',
