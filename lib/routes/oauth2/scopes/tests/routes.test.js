...
const getScopeFromResponse = (response) => response.result.oauth_client_scope.get();

describe('/oauth2/scopes route tests', () => {
  let server;
  it('should return the scope correctly', async () => {
    server = await resetAndMockDB(async (allDbs) => {
      allDbs.models.oauthClientScopes.$queryInterface.$useHandler(
        (query) => {
          if (query === 'findById') {
            return oauthClient;
          }
  ...
  it('should update the scope of client', async () => {
    server = await resetAndMockDB(async (allDbs) => {
      ...
      allDbs.models.oauthClientScopes.update = () => new Promise((resolve, reject) => resolve(oauthClient));
    });
    const res = await server.inject({
      method: 'PATCH',
      url: '/oauth2/scopes',
  ...
  it('should return badRequest if a scope of the given Id was not found', async () => {
    server = await resetAndMockDB(async (allDbs) => {
      ...
      allDbs.models.oauthClientScopes.update = () => new Promise((resolve, reject) => reject(new Error(errorMessage)));
    });
    const res = await server.inject({
      method: 'PATCH',
      url: '/oauth2/scopes',
