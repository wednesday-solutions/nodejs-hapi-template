...
describe('/oauth2/tokens/ route tests', () => {
  ...
  it('should create a new token for the after validating the parameters', async () => {
    server = await resetAndMockDB(async (allDbs) => {
      allDbs.models.oauthClients.$queryInterface.$useHandler((
        query,
      ) => {
        if (query === 'findById') {
          return oauthClient;
      ...
      allDbs.models.oauthAccessTokens.create = () => new Promise((resolve, reject) => resolve({
        get: () => ({
          accessToken: 'token',
        }),
      }));
  ...
  it('should return badImplementation if access token creation fails', async () => {
    server = await resetAndMockDB(async (allDbs) => {
      ...
      allDbs.models.oauthAccessTokens.create = () => new Promise((resolve, reject) => reject(new Error(errorMessage)));
    });
    const res = await server.inject({
      method: 'POST',
      url: '/oauth2/tokens',
